...
type Audio = "audio/wav" | "audio/mpeg" | "audio/ogg";
...
export type Accept = Image | Audio;
...
type Props = BaseProps & WithCompact;

type IpfsProps = {
  onChange?: (CID: string, mimeType: string) => void;
  onFileChange?: (arg0: File) => void;
} & Omit<Props, "uploadProgress" | "uploading" | "onChange">;

export const IpfsMediaPicker: FC<IpfsProps> = ({
  onChange,
  onFileChange,
  onReset,
  ...props
  ...
  if (type.includes("image")) {
    return (
      <img
        ...
        className={classNames(
          cover
            ? "bg-gray-100 min-h-full min-w-full object-cover object-center"
      ...
      {(() => {
        ...
        return (
          <div className="w-6 h-6">
            {(() => {
              if (uploading) return <div>loading</div>;
...
const MediaTag = ({
  ...
  if (uploading) {
    ...
    statusProps = {
      children: "Success",
      className: "text-green-400 bg-green-50",
    };
  else if (error)
    statusProps = {
...
const RemoveButton = ({ cover, onClick }: RemoveButtonProps) => {
  ...
  return (
    <button
      className="rounded-full hover:bg-gray-100 text-gray-600 p-2 transition"
      onClick={onClick}
    >
      {content}
    </button>
  ...
  return (
    ...
    >
      {label && (
        <label
          className={classNames(
            "flex items-center rounded-full h-full px-2 shadow-[0_0_0_2px_white]"
          )}
        >
          {label}
        </label>
