...
import { IpfsMediaPicker } from "./MediaPicker";
import {
  MetadataVersions,
...
const UploadPage: FC = () => {
  ...
  const [audioCID, setAudioCID] = useState<string>(null);
  const [audioType, setAudioType] = useState<string>(null);
  ...
  const uploadVideo: FormEventHandler<HTMLFormElement> = async (event) => {
    ...
    if (!audioCID)
      return toast.error("Please wait for the track to finish uploading.");
    if (!thumbnailCID)
      return toast.error("Please wait for the thumbnail to finish uploading.");
    ...
    const waitForIndex = await createPost({
      ...
      media: [
        {
          item: `ipfs://${audioCID}`,
          type: audioType as VideoMimeTypes,
          cover: `ipfs://${thumbnailCID}`,
        },
      ],
    });
    ...
    <div className="container mx-auto py-12 px-4 md:px-0">
      <div>
        ...
        <p className="mt-2 text-sm text-gray-500">
          Uploaded videos will be visible on your profile, search and explore
          pages. Other Lens-powered sites may choose to show them as well.
        </p>
      </div>
      <form
          ...
          <div>
            <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
              ...
              <div className="sm:col-span-6">
                ...
                <p className="mt-2 text-sm text-gray-500">
                  Will be shown below the video, across Lens sites, and on the
                  collected NFT.
                </p>
              </div>
            </div>
                ...
                <label className="relative flex items-start">
                  ...
                  <div className="ml-3 text-sm">
                    ...
                    <p id="rights-description" className="text-gray-500">
                      I own the rights for the uploaded content, and allow
                      Lumiere (and other Lens-powered sites) to host and display
                      it
                    </p>
                  </div>
                </label>
                <label className="relative flex items-start">
                  ...
                  <div className="ml-3 text-sm">
                    ...
                    <p id="sensitive-description" className="text-gray-500">
                      The uploaded content is suitable for all audiences, and
                      does not contain illegal or sensitive media
                    </p>
                  </div>
                </label>
              </div>
