...
const useReactToPublication = (
  ...
  const { data: reactedData, refetch } = useQuery<
    {
      publication: {
        reaction: ReactionTypes;
        stats: { totalUpvotes: number; totalDownvotes: number };
      };
    },
  ...
  const reactToPublication = useCallback(
    async (publicationId: string, reaction: ReactionTypes) => {
      ...
      return await toastOn(
        ...
        {
          loading: `${
            reaction == ReactionTypes.Upvote ? "Liking" : "Disliking"
          }...`,
          success: `${
          ...
          error: (err) => {
            ...
            if (!err.message.startsWith("You have already reacted"))
              return err?.message;
  ...
  const removeReaction = useCallback(
    async (publicationId: string, reaction: ReactionTypes) => {
      ...
      return await toastOn(
        ...
        {
          loading: `Removing ${
            reaction == ReactionTypes.Upvote ? "like" : "dislike"
          }...`,
          success: `Removed!`,
  ...
  return {
    ...
    upvotePublication: (publicationId: string) =>
      reactToPublication(publicationId, ReactionTypes.Upvote),
    downvotePublication: (publicationId: string) =>
      reactToPublication(publicationId, ReactionTypes.Downvote),
  };
};
