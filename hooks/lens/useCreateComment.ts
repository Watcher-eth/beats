...
import { Mutation, MutationCreateCommentTypedDataArgs, RelayerResult } from '../../types/index'

type CreateComment = {
	createComment: (url: string) => Promise<() => Promise<unknown>>
	loading: boolean
	error?: Error
}
...
console.log(url)
			...
			if (RELAYER_ON) {
				const result = await toastOn(
					...
					{ loading: 'Sending Comment...', success: 'Comment sent!', error: ERROR_MESSAGE }
				)
