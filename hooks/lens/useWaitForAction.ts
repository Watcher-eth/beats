...
type WaitForAction = {
  ...
  waitForAction: ({
    ...
    txId?: string;
    txHash?: string;
  }) => Promise<Maybe<TransactionResult>>;
  resolveOnAction: ({
    txId,
    txHash,
  }: {
...
const useWaitForAction = ({
  ...
  const {
    ...
    loading,
    refetch,
  } = useQuery<{ tx: TransactionResult }, QueryHasTxHashBeenIndexedArgs>(
    INDEXED_REQ,
    {
      variables: { request: txId ? { txId } : { txHash } },
      skip: !txId && !txHash,
  ...
  useEffect(() => {
    ...
    if (
      data.tx.metadataStatus?.status ===
        PublicationMetadataStatusType.Success &&
      onParse
    ) {
  ...
  const metadataStatus = useMemo<PublicationMetadataStatusType>(
    () =>
      data?.tx?.__typename == "TransactionIndexedResult" &&
      data?.tx?.metadataStatus?.status,
    [data]
  );
  const error = useMemo(
  ...
  const waitForAction = useCallback(
    async ({ txId: _txId, txHash: _txHash }) => {
      if (!txId && !txHash && !_txId && !_txHash)
        throw new Error("Transaction details not provided.");

      while (true) {
        try {
          const res = await hasTxBeenIndexed({
            txId: _txId || txId,
