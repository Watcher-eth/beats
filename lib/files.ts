export const validateAccept = (
  fileType: string,
  accept: React.AllHTMLAttributes<HTMLInputElement>["accept"]
) => {
...
export const dataURItoFile = (dataURI: string, fileName: string): File => {
  const [, , type, , encodedByteString] =
    /^(data:)([\w\/\+-]*)(;charset=[\w-]+|;base64){0,1},(.*)/gi.exec(dataURI);
  const byteString = Buffer.from(encodedByteString, "base64").toString(
    "binary"
  );
