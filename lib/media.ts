import { Media, MediaSet, NftImage } from "../types/index";

export const getImageUrl = (media: MediaSet[]): string => {
  const image = media.find(({ original: { mimeType } }) =>
...
export const getVideo = (media: MediaSet[]): Media | null => {
  const video = media.find(({ original: { mimeType } }) =>
...
export const normalizeUrl = (url: string, mimeType?: string): string => {
  ...
  if (parsed.protocol == "ipfs:") {
    return `https://lens.infura-ipfs.io/ipfs/${
