...
import { ProfileMetadata } from "../types/local";

export const classNames = (...classes: string[]): string =>
  classes.filter(Boolean).join(" ");

export const omit = <T>(object: T, name: string): Omit<T, "__typename"> =>
  omitDeep(object, name);

export const unCapitalize = (value: string): string =>
  value.charAt(0).toLowerCase() + value.slice(1);

export const trimIndentedSpaces = (value: string): string =>
  value?.replace(/\n\s*\n/g, "\n\n").trim();
